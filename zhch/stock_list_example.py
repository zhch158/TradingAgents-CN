...
try:
    ...
    print(f"⚠️ enhanced_stock_list_fetcher 模块不可用: {e}")
    print("请检查 utils/enhanced_stock_list_fetcher.py 是否存在")
...
def demo_stock_list_fetcher():
    ...
    if not ENHANCED_FETCHER_AVAILABLE:
        print("❌ enhanced_stock_list_fetcher 模块不可用，无法运行演示")
        return
...
if __name__ == "__main__":
    ...
    if ENHANCED_FETCHER_AVAILABLE:
        ...
        print("❌ enhanced_stock_list_fetcher 模块不可用")
        print("请检查以下文件是否存在：")
        print("- utils/enhanced_stock_list_fetcher.py")
        print("- data/scripts/enhanced_stock_list_fetcher.py")